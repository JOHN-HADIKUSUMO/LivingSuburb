...
namespace LivingSuburb.ScienceWinService
{
    public class RunServices:ServiceBase
    {
        ...
        public RunServices()
        {
            ServiceName = "Science News Service";
        ...
        private async void RunScienceNews()
        {
            logger.LogInformation("RunScienceNews starts at " + Environment.TickCount);
            ...
            if (newsService != null)
            {
                NewsAPIModel science = await newsService.GetHTTPNews(CountriesEnum.Australia.ToString(), Category.Science.ToString(), DateTime.Now.AddDays(-1), DateTime.Now);
                if (science != null)
                {
                    for (int i = 0; i < science.articles.Count() - 1; i++)
                    {
                        News news = new News();
                        news.NewsTitle = science.articles[i].title;
                        news.NewsURL = science.articles[i].url;
                        news.NewsSource = science.articles[i].source.name;
                        news.NewsCategory = Category.Science.ToString();
                        news.NewsCountry = CountriesEnum.Australia.ToString();
                        DateTime tempDate;
                        if (DateTime.TryParse(science.articles[i].publishedAt, out tempDate))
                        {
            ...
            logger.LogInformation("RunScienceNews stops at " + Environment.TickCount);
        }
