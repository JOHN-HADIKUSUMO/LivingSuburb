...
namespace LivingSuburb.PoliticWinService
{
    public class RunServices:ServiceBase
    {
        ...
        public RunServices()
        {
            ServiceName = "Politic News Service";
        ...
        private async void RunPoliticNews()
        {
            logger.LogInformation("RunPoliticNews starts at " + Environment.TickCount);
            ...
            if (newsService != null)
            {
                NewsAPIModel politic = await newsService.GetHTTPNews(CountriesEnum.Australia.ToString(), Category.Politics.ToString(), DateTime.Now.AddDays(-1), DateTime.Now);
                if (politic != null)
                {
                    for (int i = 0; i < politic.articles.Count() - 1; i++)
                    {
                        News news = new News();
                        news.NewsTitle = politic.articles[i].title;
                        news.NewsURL = politic.articles[i].url;
                        news.NewsSource = politic.articles[i].source.name;
                        news.NewsCategory = Category.Politics.ToString();
                        news.NewsCountry = CountriesEnum.Australia.ToString();
                        DateTime tempDate;
                        if (DateTime.TryParse(politic.articles[i].publishedAt, out tempDate))
                        {
            ...
            logger.LogInformation("RunPoliticNews stops at " + Environment.TickCount);
        }
