...
namespace LivingSuburb.EntertainmentWinService
{
    public class RunServices:ServiceBase
    {
        ...
        public RunServices()
        {
            ServiceName = "Entertainment News Service";
        ...
        {
            Task.Run(() => {
                RunEntertainmentNews();
            });
        }

        private async void RunEntertainmentNews()
        {
            logger.LogInformation("RunEntertainmentNews starts at " + Environment.TickCount);
            ...
            if (newsService != null)
            {
                NewsAPIModel model = await newsService.GetHTTPNews(CountriesEnum.Australia.ToString(), Category.Entertainment.ToString(), DateTime.Now.AddDays(-1), DateTime.Now);
                if (model != null)
                {
                    for (int i = 0; i < model.articles.Count() - 1; i++)
                    {
                        ...
                        news.NewsCategory = Category.Entertainment.ToString();
            ...
            logger.LogInformation("RunEntertainmentNews stops at " + Environment.TickCount);
        }
